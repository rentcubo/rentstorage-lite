                                    ...
                                    <div class="template-demo">
                                        ...
                                        <table class="table mb-0">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td>
                                                        <a class="btn btn-primary" href="{{ route('admin.users_edit', ['user_id' => $user_details->id])}}">{{tr('edit')}}</a>

                                                        <a class="btn btn-danger" href="{{route('admin.users_delete', ['user_id' => $user_details->id])}}" onclick="return confirm(&quot;{{tr('user_delete_confirmation' , $user_details->name)}}&quot;);">{{tr('delete')}}</a> 
                                                        @if($user_details->status == USER_APPROVED)

                                                            <a class="btn btn-danger" href="{{ route('admin.users_status', ['user_id' => $user_details->id]) }}" onclick="return confirm(&quot;{{$user_details->first_name}} - {{tr('user_decline_confirmation')}}&quot;);">
                                                                {{ tr('decline') }} 
                                                            </a> 
                                                        @else

                                                            <a class="btn btn-success" href="{{ route('admin.users_status', ['user_id' => $user_details->id]) }}">
                                                                {{ tr('approve') }} 
                                                            </a> 
                                                        @endif
                                                    </td>
