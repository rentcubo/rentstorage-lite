...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item"><i class="fa fa-share"></i> &nbsp<a href="{{route('admin.bookings_index')}}">&nbsp{{tr('bookings')}}</a></li>
          		  	...
          		  	<div class="d-flex justify-content-between align-items-center">
	          		    ...
	          		    <div class="d-inline-block">
	          		      	<div class="px-3 px-md-4 py-2 rounded text-uppercase text-success">
	 		                	...
	 		                	<a href="{{ route('admin.users_view', ['user_id' => $booking_details->user_id ]) }}">{{ $booking_details->users ? $booking_details->users->name : tr('user_not_avail') }}</a>

	 		                	<h4 class="card-title">{{ tr('provider') }}</h4>
	 		                	<a href="{{ route('admin.providers_view', ['provider_id' => $booking_details->provider_id ]) }}">{{ $booking_details->providers ? $booking_details->providers->name: tr('provider_not_avail') }}</a>

	 		                	<h4 class="card-title">{{ tr('space') }}</h4>
	 		                	<a href="{{ route('admin.spaces_view', ['space_id' => $booking_details->space_id ]) }}">{{ $booking_details->spaces ? $booking_details->spaces->name : tr('space_not_avail') }}</a>
				...
				<div class="box-body">
					<div class="preview-list">
						...
						<div class="preview-item border-bottom px-0">
							...
							<div class="preview-item-content d-flex flex-grow">
								<div class="flex-grow">
									<h4 class="preview-subject">{{tr('duration')}}
										<span class="float-right small">
											<span class="text-muted pr-3 pull-right">{{ $booking_details->duartion }}</span>
										</span>
									</h4>
								</div>
							</div>
							            ...
							            <tr>
							              	...
							              	<td class="pr-0 text-right">
								              	<div class="badge badge-danger">
								              		{{$booking_payment_details->total_time}}
								              	</div>
							              	</td>
							            </tr>
				                        <tr>
